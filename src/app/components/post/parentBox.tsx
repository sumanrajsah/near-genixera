...
export default function ParentBox({ post }: { post: PostInfo }) {
...
<TopLoader/>
      <div className="posts-box" style={{borderBottomLeftRadius:'0px',borderBottomRightRadius:'0px',borderBottom:'0px',borderStyle:'dashed'}} onClick={() => setShowPostDrop(false)} ref={postRef}>
        ...
        <div className="post-body" onClick={showPost}>
          ...
          {(post.post_type === 'gif') && <div className="view-image" onClick={(e) => {e.stopPropagation(),setZoomImg(true)}}>  {MediaBlob&&<Image loading="lazy" alt='genx' className="img" src={URL.createObjectURL(MediaBlob)} height={1000} width={1000} unoptimized/>} </div>}
           {(post.post_type === '')&& <div className="view-video" onClick={(e) => e.stopPropagation()}><video src={`${process.env.NEXT_PUBLIC_API_IPFS_URL}/${post.media_url.split('/')[4]}`} className="video-bg" height={1000} width={1000} />
           <LazyLoad className="video-l"><ReactPlayer style={{}} muted  controls playing={(postInView?(true):(false))} url={`${process.env.NEXT_PUBLIC_API_IPFS_URL}/${post.media_url.split('/')[4]}`} /></LazyLoad>
           </div> }
          ...
          <div className="zoom-img-cont" onClick={(e) => setZoomImg(false)}>
            ...
            <Image src={`${process.env.NEXT_PUBLIC_API_IPFS_URL}/${post.media_url.split('/')[4]}`} height={1000} width={1000} className="zoom-img" alt={post.author_username} onClick={(e) => e.stopPropagation()} title={post.author_username}/>
          </div>}
      <div className="action-bar" style={{borderRadius:'0px',borderTop:'0px',borderBottom:'0px',borderStyle:'dashed'}}>
